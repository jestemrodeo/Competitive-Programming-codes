...
void BFS(vector < vector <long> > &grafo, vector < vector <long> > &distancia, vector < vector <string> > &brujula, long inicio, vector <long> &nodoDistancia, vector <long> &padre){
...
void caminoMasLargo(vector < vector <long> > &grafo, vector < vector <long> > &distancia, vector < vector <long> > &brujula){
    //Maximo
    vector <int> nodoMaximo;
    vector <string> sentidoMaximo;
    //Auxiliar
    vector <int> nodoAuxiliar;
    vector <string> sentidoAuxiliar;

    vector <long> nodoDistancia(grafo.size(),INFINITO);
    vector <long> padre(grafo.size(),-1);

    BFS(grafo,distancia,brujula,0,nodoDistancia,padre);
    ...
    {
        int a=(camino[c1]-1)/barrio.size();
        int b=(camino[c1]-1)-barrio.size()*a;
        cout<<a<<" "<<b<<" - ";
    }
    cout<<endl;
}
