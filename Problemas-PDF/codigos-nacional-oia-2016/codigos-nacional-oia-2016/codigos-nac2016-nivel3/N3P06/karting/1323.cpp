...
struct punto{
	unsigned int i,j,maxVecino,oldi,oldj,yo;
};
...
{		
	...
	for(unsigned int i = 0; i < M; i++){
		for(unsigned int j = 0; j < M; j++){
				if(barrio[i][j]==max){// me fijo cuantas altas tengo
					punto temp;
					temp.i = i,temp.j = j,temp.maxVecino = 0,temp.oldi = i,temp.oldj = j,temp.yo=barrio[i][j];					
    ...
    for(int p = 0; p < puntos.size(); p++){	
		...
		while(puntos[p].maxVecino!=1){
			...
			for(int i=-1;i<2;i++){
				if(i == 0) continue;
				if((puntos[p].i+i)<0) continue;
				if((puntos[p].i+i)>barrio.size()) continue;					
					if(barrio[puntos[p].i+i][puntos[p].j] > puntos[p].maxVecino){
						if(puntos[p].maxVecino < puntos[p].yo){
			...
			for(int i=-1;i<2;i++){
				if(i == 0) continue;
				if((puntos[p].j+i)<0) continue;
				if((puntos[p].j+i)>barrio[0].size()) continue;					
					if(barrio[puntos[p].i][puntos[p].j+i] > puntos[p].maxVecino){
						if(puntos[p].maxVecino < puntos[p].yo){
