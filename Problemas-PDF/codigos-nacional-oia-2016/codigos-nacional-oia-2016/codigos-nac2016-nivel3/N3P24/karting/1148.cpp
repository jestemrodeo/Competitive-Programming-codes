...
string recorrer(int x, int y, vector<vector<int>> &barrio, vector<vector<int>> &pista, int &largopista, int M){
	...
	if(proximo.altura>1){
		...
		for(int point=0;point<recorridov.size();point++){
			if(recorridov[point].y<y) recorridoc[point]='N'; if(recorridov[point].y>y) recorridoc[point]='S';
			if(recorridov[point].x<x) recorridoc[point]='O'; if(recorridov[point].x>x) recorridoc[point]='E';
		}
