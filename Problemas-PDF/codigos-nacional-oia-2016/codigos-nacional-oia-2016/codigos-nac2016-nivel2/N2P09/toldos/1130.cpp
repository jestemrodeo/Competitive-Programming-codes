...
void bfs(const vector<int> costos){
    ...
    while(!Q.empty()){
        ...
        for(int i=0;i<=costos.size();i++){
            if( puestos[act.first.first] != true && i==costos.size() && act.first.first+1 <= p){
                Q.push( make_pair( make_pair(act.first.first+1,act.first.second),act.second ) );
            } else if( i!=costos.size() && act.first.first+(i+1) <= p){
                Q.push( make_pair( make_pair( act.first.first+(i+1),act.first.second+costos[i] ), act.second+puestospasados(act.first.first,i+1) ) );
            }
        }
