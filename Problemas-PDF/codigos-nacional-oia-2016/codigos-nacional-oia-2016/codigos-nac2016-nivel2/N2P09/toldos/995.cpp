...
void bfs(const vector<int> costos){
    ...
    while(!Q.empty()){
        ...
        cout << act.first.first << ' ' << act.first.second << ' ' << p << endl;
        ...
        for(int i=0;i<costos.size();i++){
            if( puestos[act.first.first] != true && i==0 && act.first.first+1 <= p){
                ...
                Q.push( make_pair( make_pair( act.first.first+i,act.first.second+costos[i] ), act.second+puestospasados(act.first.first,i) ) );
            }
        }
