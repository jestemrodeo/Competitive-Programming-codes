...
int toldos(vector<int> feriantes, vector<int> costos)
{
    ...
    if((mayor-menor)*costos[0]<costos[mayor-menor] &&(mayor-menor)*costos[0]<(mayor-menor)/2*costos[1]){
        return (mayor-menor)*costos[0];
    }else if((mayor-menor)/2*costos[(mayor-menor)/2]<costos[mayor-menor]){
        return (mayor-menor)/2*costos[(mayor-menor)/2];
    }else {
